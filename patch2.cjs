...
let content = fs.readFileSync('src/components/ConceptExplanation.jsx', 'utf8').replace(/\r/g, '');

const c3Target = `  const handleAudioPlay = () => {
...
const c3Repl = `  const handleAudioPlay = () => {
    console.log("Playing audio for current text...");
  };
...
const c4Target = `              {/* Mic Button Area */}
...
content = content.replace(c3Target, c3Repl).replace(c4Target, c4Repl);

if (!content.includes('startListening = () =>')) {
    console.log("FAILED to patch C3");
}
if (!content.includes('onClick={startListening}')) {
    console.log("FAILED to patch C4");
}
